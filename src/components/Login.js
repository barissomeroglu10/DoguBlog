...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setMessage(err.message || 'Giriş başarısız. Lütfen bilgilerinizi kontrol edin.');
      setMessageType('error');
    } finally {
      setIsLoading(false);
    }
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      if (result.success) {
        setMessage('Google ile giriş başarılı!');
        setMessageType('success');
        setTimeout(() => {
          navigate(from, { replace: true });
        }, 1000);
      ...
      setMessage(err.message || 'Google ile giriş başarısız.');
      setMessageType('error');
    } finally {
      setIsLoading(false);
    }
    ...
    <div className="bg-gradient-to-br from-blue-50 to-indigo-100 min-h-screen flex items-center justify-center p-4">
      <div className="max-w-md w-full">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600">Tekrar hoş geldin!</p>
        </div>
        <div className="bg-white rounded-2xl shadow-xl p-8">
          <h2 className="text-2xl font-semibold text-gray-900 mb-6 text-center">Giriş Yap</h2>
          {(message || error) && (
        ...
        <div className="mt-6 text-center text-xs text-gray-400">
          <p>Firebase bağlantı hatası veya girişte sorun yaşarsan, <b>config.js</b> dosyasındaki anahtarları ve Firebase Console ayarlarını kontrol et.</p>
        </div>
      </div>
    </div>
  );
